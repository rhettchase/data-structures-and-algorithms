...
const typeNum = (arr) => {
  return arr.filter((dataType) => typeof dataType === "number");
};
...
// filter callback checks if each element in the arr array is NOT included in the forbiddenValues array, and only those elements are included in the result.
...
const getStatName = (arr, minBaseStat) => {
  let filteredObjects = getBaseStatGreaterThan(arr, minBaseStat);
  let names = filteredObjects.map((obj) => obj.stat.name)
  return names;
};
