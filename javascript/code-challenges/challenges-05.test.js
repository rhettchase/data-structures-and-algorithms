...
const getHouses = (arr) => {
  ...
  // Use the map function to iterate over each object in the input array
  // Extract the 'house' property from each object using destructuring, returning array of house values
  houses = arr.map(({ house }) => house);
  return houses;
};
...
const hasChildrenValues = (arr, character) => {
  // checks if the `name` property of each character matches the specified `character` parameter
  for (const char of arr) {
    if (char.name === character) {
      const values = Object.values(char);
      // checks if the character has children
