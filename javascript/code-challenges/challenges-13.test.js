...
const sortByChildren = (charArray) => {
  // Sort characters by the number of children and then alphabetically by house name
  characters.sort((a, b) => {
    // Compare the number of children
    const numChildrenComparison = a.children.length - b.children.length;

    if (numChildrenComparison !== 0) {
      // If the number of children is different, return the comparison result
      return numChildrenComparison;
    } else {
      // If the number of children is the same, compare alphabetically by house name
...
const containsW = (str) => {
  // Use a regular expression to check for the letter 'w' in lowercase
  const pattern = /w/;
...
const isNum = (input) => {
  // Use a regular expression to check if the input contains a digit
  const pattern = /\d/;
...
const containsWorld = (input) => {
  // Use a regular expression to check if the input contains the word 'world'
...
const citiesAtoJ = (arr) => {
  // Use a regular expression to filter cities that begin with A to J
  const pattern = /^[A-J]/i; // ^[A-J] matches any string that starts with the letters A through J (case-insensitive)
