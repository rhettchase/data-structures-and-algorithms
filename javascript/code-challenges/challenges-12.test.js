...
const count = (target, input) => {
  return input.reduce((acc, currentArray) => {
    // Use the filter method to count occurrences of the target in the currentArray
    const occurrences = currentArray.filter(item => item === target).length;
    // Add the occurrences to the accumulator
...
const totalSum = (input) => {
  return input.reduce((acc, currentArray) => {
    // Use the reduce method to sum up the elements in the currentArray
    const arraySum = currentArray.reduce((sum, element) => sum + element, 0);

    // Add the arraySum to the accumulator
    return acc + arraySum;
  }, 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map((innerArray) => {
    // Use filter to remove non-numbers and numbers not divisible by five
    const filteredArray = innerArray.filter((element) => typeof element === 'number' && element % 5 === 0);

    // Use map to raise 2 to the power of each element in the filteredArray
    const poweredArray = filteredArray.map((element) => Math.pow(2, element));
...
let findMaleAndFemale = (data) => {
  // Use filter to select characters with gender 'male' or 'female'
  const selectedCharacters = data.filter(character => character.gender === 'male' || character.gender === 'female');

  // Use map to extract names from the selected characters
  const characterNames = selectedCharacters.map(character => character.name);

  // Use join to combine names into a single string separated by "and"
...
let findShortest = (data) => {
  ...
  const shortestCharacter = data.reduce((minHeightCharacter, currentCharacter) => {
    // Convert height strings to numbers for comparison
    const minHeight = parseFloat(minHeightCharacter.height);
    const currentHeight = parseFloat(currentCharacter.height);

    // If current character's height is shorter than the stored minimum, update the minimum
    if (isNaN(minHeight) || isNaN(currentHeight) || currentHeight < minHeight) {
      return currentCharacter;
    } else {
      return minHeightCharacter;
    }
  }, {});
